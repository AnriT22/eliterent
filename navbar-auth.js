    ...
    function initNavbarAuth() {
        ...
        if (token && user) {
            ...
            var profileHTML = ''
                ...
                + (user.role === 'partner'
                    ? (user.is_verified
                        ? '<span style="display:inline-flex;align-items:center;gap:3px;padding:2px 8px;background:rgba(34,197,94,0.15);color:var(--tt-green2, #22c55e);border-radius:12px;font-size:11px;font-weight:600;margin-top:4px;"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path d="M20 6 9 17l-5-5"/></svg>' + t('nav.verified','Verified') + '</span>'
