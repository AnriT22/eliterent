...
(function () {
    ...
    function injectStyles() {
        ...
        var css =
            ...
            + '.cpop-card{position:relative;width:100%;max-width:380px;background:linear-gradient(160deg,var(--th-panel, #15171f),var(--th-surface, #1c1e26));'
            ...
            + 'background:var(--tf-08, rgba(255,255,255,0.08));color:#cbd0dc;font-size:18px;line-height:1;cursor:pointer;transition:background .15s,transform .2s;}'
            ...
            + '.cpop-title{margin:0 0 4px;font-size:19px;font-weight:800;color:var(--tt-gold5, #F4E3B2);letter-spacing:.2px;}'
            + '.cpop-sub{margin:0 0 18px;font-size:13px;color:var(--tt-muted, #A0A3B0);}'
            + '.cpop-grid{display:flex;flex-direction:column;gap:9px;}'
            + '.cpop-country{display:flex;align-items:center;gap:12px;width:100%;padding:11px 14px;background:var(--tf-04, rgba(255,255,255,0.04));'
            + 'border:1px solid var(--tb-border2, #2A2D38);border-radius:12px;cursor:pointer;transition:border-color .15s,background .15s,transform .15s;text-align:left;}'
            + '.cpop-country:hover{border-color:var(--tb-gold2, #D4AF37);background:rgba(212,175,55,.08);transform:translateY(-1px);}'
            + '.cpop-flag{font-size:22px;line-height:1;}'
            + '.cpop-name{flex:1;font-size:15px;font-weight:600;color:var(--tt-text, #EAEAEA);}'
            + '.cpop-soon{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.04em;color:var(--tt-gold, #D4AF37);'
