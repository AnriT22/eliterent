...
router.get('/overview', authenticateToken, requireRole('partner'), function(req, res) {
    try {
        ...
        if (period !== 'all') {
            ...
            dateFilter = "AND b.created_at >= datetime('now', '-" + days + " days')";
        }

        var activeVehiclesRow = queryOne(
            "SELECT COUNT(*) as count FROM vehicles WHERE partner_id = ? AND status = 'active'",
            [partnerId]
        );

        var bookingsRow = queryOne(
            "SELECT COUNT(*) as count, COALESCE(SUM(b.rental_days), 0) as total_days " +
            "FROM bookings b WHERE b.partner_id = ? AND b.status IN " + EARNING_STATUSES + " " + dateFilter,
            [partnerId]
        );

        var upcomingRow = queryOne(
            "SELECT COUNT(*) as count FROM bookings b " +
            "WHERE b.partner_id = ? AND b.status = 'accepted' AND b.pickup_date >= date('now')",
            [partnerId]
        );

        var earningsRow = queryOne(
        ...
        if (period !== 'all') {
            ...
            var prevRow = queryOne(
                "SELECT COALESCE(SUM(b.total_price * " + PARTNER_SHARE + "), 0) as total " +
                "FROM bookings b WHERE b.partner_id = ? AND b.status IN " + EARNING_STATUSES + " " +
                "AND b.created_at >= datetime('now', '-" + (d * 2) + " days') " +
                "AND b.created_at < datetime('now', '-" + d + " days')",
                [partnerId]
            );
        ...
        var earningsData = generateEarningsData(partnerId, period);
        var statusData = generateStatusData(partnerId);
        var recentBookings = getRecentBookings(partnerId, 20);

        res.json({
            stats: {
                totalEarnings: Math.round(currentEarnings * 100) / 100,
                activeVehicles: (activeVehiclesRow && activeVehiclesRow.count) || 0,
                completedBookings: (bookingsRow && bookingsRow.count) || 0,
                completedDays: Math.round((bookingsRow && bookingsRow.total_days) || 0),
                upcomingBookings: (upcomingRow && upcomingRow.count) || 0,
...
function generateEarningsData(partnerId, period) {
...
function generateStatusData(partnerId) {
    ...
    statuses.forEach(function(status) {
        var row = queryOne(
            "SELECT COUNT(*) as count FROM bookings WHERE partner_id = ? AND status = ?",
            [partnerId, status]
        );
...
function getRecentBookings(partnerId, limit) {
